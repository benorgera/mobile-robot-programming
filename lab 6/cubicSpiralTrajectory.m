classdef cubicSpiralTrajectory < handle
    ...
    methods(Static = true)
        ...
        function curve = planTrajectory(x,y,th,sgn,worldCoords)
            ...
            if (nargin == 5 && worldCoords)
                ...
                x = deltaPRob(1);
                y = deltaPRob(2);
                th = atan2(sin(th - p0(3)), cos(th - p0(3)));
            else
                p0 = [0;0;0];
            end
            ...
            clothu = cubicSpiralTrajectory([au bu su],201,p0);
    ...
    methods(Access = private)
        ...
        function integrateCommands(obj)
            ...
            for i=1:obj.numSamples-1
                ...
                k = s * (a + b*s)*(s - sf);
                obj.r = obj.r + k*k*ds;
                theta = obj.poseArray(3, i) + ds * k;
                if (abs(theta) > obj.maxHeading)
                    obj.distArray = [];
                    return
                end
                obj.curvArray(i+1) = k;
                obj.distArray(i+1) = obj.distArray(i) + ds;
                obj.poseArray(1, i+1) = obj.poseArray(1, i) + cos(theta)*ds;
                obj.poseArray(2, i+1) = obj.poseArray(2, i) + sin(theta)*ds;
    ...
    methods(Access = public)
        ...
        function obj = cubicSpiralTrajectory(parms,numSamples,p0)
        ...
        function poseOut  = getPoseAtDist(obj,s)
